...
if agent is not None and state is not None:
    ...
    # ATUALIZA A SUGESTÃO DE VIZINHOS COM A TABELA DE PUXAR
    if len(st.session_state.history) > 10: # Para garantir que a tabela tenha dados suficientes
        all_suggestions = get_smart_neighbor_suggestions(top_actions, st.session_state.pull_table, n_neighbors=2)
    else:
        all_suggestions = top_actions
else:
    top_actions = random.sample(range(NUM_TOTAL), 3)
    all_suggestions = top_actions

st.subheader("🤖 Ações sugeridas pela IA (DQN) + vizinhos dinâmicos")
st.write("A IA sugere os números principais e também aqueles que a 'tabela de puxar' indicou como prováveis. Sua aposta deve incluir ambos:")
for action in all_suggestions:
    st.write(f"Sugestão de Aposta: **{action}**")

st.session_state.prev_state = state
st.session_state.prev_action = all_suggestions[0] if all_suggestions else None
st.session_state.prev_actions = all_suggestions[:] if all_suggestions else None
...
st.json(st.session_state.pull_table)
